...
void __fastcall TForm1::PaintBox1MouseDown(TObject *Sender, TMouseButton Button, TShiftState Shift,
		  int X, int Y)
{
	// Sprawdzenie gdzie klinkal gracz
	int podzialX = PaintBox1->Width / 3;
	int podzialY = PaintBox1->Height / 3;
	int rozmiar = 0;
	int x;
	...
	// Gra Online (wyslanie informacji o zmianie tury)
	if (online) {
		// Jezeli gracz jest serwerem
		if (serverMode) {
			serverSendMessage( "/nextTurn " + IntToStr(x) + ":" + IntToStr(y) );
		// Jezeli gracz jest klientem
		} else {
			clientSendMessage( "/nextTurn " + IntToStr(x) + ":" + IntToStr(y) );
		}
    }

	// Jezeli jest wygrana, wyœwietl komunikat
...
void TForm1::reset_gry() {
	...
	// Konfiguracja gry Online (poczatek wybranie graczy)
	if (online) {
	...
	// Konfiguracja gry Offline (poczatek wybieranie gracza)
	} else {
		gracz = 'o';
    }
}
...
//---------------------------------------------------------------------------
char TForm1::sprawdz(){
    //Sprawdzenie czy gracz nie wygral
...
void TForm1::serverStart() {
	try {
        ...
        Konsola->Lines->Add( "( ! ) Blad podczas konfiguracji serwera !" );
		MessageBox(NULL, L"Blad podczas konfiguracji serwera", L"Blad Serwera", MB_ICONERROR + MB_OK);
	}
	...
	try {
        ...
        Konsola->Lines->Add( "( ! ) Blad podczas uruchamiania serwera !" );
		MessageBox(NULL, L"Blad podczas uruchamiania serwera", L"Blad Serwera", MB_ICONERROR + MB_OK);
	}
}
//---------------------------------------------------------------------------
void TForm1::serverStop() {
	try {
		server->Active = false;
		serverMode = false;
        Konsola->Lines->Add( "( i ) Serwer wylaczony !" );
	} catch( Exception *exc ) {
		Konsola->Lines->Add( "( ! ) Blad podczas wylaczania serwera !" );
		MessageBox(NULL, L"B³lad podczas wylaczania serwera", L"Blad Serwera", MB_ICONERROR + MB_OK);
	}
}
//---------------------------------------------------------------------------
void __fastcall TForm1::serverDetectConnect(TIdContext *AContext) {
	try {
		...
		Konsola->Lines->Add( "( i ) Klient wlasnie sie podlaczyl !" );
		Konsola->Lines->Add( "Adres IP: " + adresIP );

		////////////////////////////////////////////////////////////////////
		// Jeseli gracz sie podlaczy po wejsciu do -> gry wyslanie konfiguracji
        ...
        Konsola->Lines->Add( "( ! ) Blad podczas podlaczania klienta !" );
        MessageBox(NULL, L"Blad podczas podlaczania klienta", L"Blad Serwera", MB_ICONERROR + MB_OK);
    }
}
//---------------------------------------------------------------------------
void __fastcall TForm1::serverDetectDisconnect(TIdContext *AContext) {
    try {
		String adresIP = AContext->Connection->Socket->Binding->IP;
		Konsola->Lines->Add( "( i ) Klient wlasnie sie odlaczyl !" );
		Konsola->Lines->Add( "Adres IP: " + adresIP );
	} catch( Exception *exc ) {
		Konsola->Lines->Add( "( ! ) Blad podczas odlaczania klienta !" );
		MessageBox(NULL, L"Blad podczas odlaczania klienta", L"Blad Serwera", MB_ICONERROR + MB_OK);
	}
}
//---------------------------------------------------------------------------
void __fastcall TForm1::serverReadMessage(TIdContext *AContext) {
	...
	try {
		...
		Konsola->Lines->Add( "( ! ) Blad podczas odbierania wiadomoœci !" );
		MessageBox(NULL, L"Blad podczas odbierania wiadomosci", L"Blad Serwera", MB_ICONERROR + MB_OK);
	}
...
void TForm1::serverSendMessage( String wiadomosc ) {
	...
	try {
		...
		Konsola->Lines->Add( "( ! ) Blad podczas wysylania wiadomosci !" );
		MessageBox(NULL, L"Blad podczas wysylania wiadomoœci", L"Blad Serwera", MB_ICONERROR + MB_OK);
	}
...
void TForm1::clientStart() {
	try { clientConfigure(); }
		...
		Notebook1->ActivePage = "MainMenu";
		MessageBox(NULL, L"Blad podczas konfiguracji klienta", L"Blad Klienta", MB_ICONERROR + MB_OK);
	}

	try { client->Connect(); }
	catch( Exception *exc ) {
		Notebook1->ActivePage = "MainMenu";
		MessageBox(NULL, L"Blad podczas podlaczania do serwera", L"Blad Klienta", MB_ICONERROR + MB_OK);
	}
}
//---------------------------------------------------------------------------
void TForm1::clientStop() {
	try { client->Disconnect(); }
	catch( Exception *exc ) { MessageBox(NULL, L"Blad podczas odlaczania klienta", L"Blad Klienta", MB_ICONERROR + MB_OK); }
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::clientReadMessage(TObject *Sender) {
	...
	try {
		...
		clientThread->Enabled = false;
        MessageBox(NULL, L"Blad podczas odbierania wiadomosci", L"Blad Klienta", MB_ICONERROR + MB_OK);
        Notebook1->ActivePage = "MainMenu";
		return;
	}
	...
	if (wiadomosc != "") {
		...
		if (wiadomosc.SubString(0,11) == "/initPlayer" ) {
			...
			MessageBox(NULL, L"Serwer rozpoczla gre", L"Rozgrywka", MB_ICONEXCLAMATION + MB_OK);
		...
		if (wiadomosc.SubString0(0,9) == "/nextTurn" ) {
            ...
            // Je¿eli jest wygrana, wyswietl komunikat
			int wynik = wygranaKomunikat( wygranyGracz );
			if (wynik == 0) {
				Notebook1->ActivePage = "MainMenu";
				clientStop();
...
void TForm1::clientSendMessage( String wiadomosc ) {
	...
	catch( Exception *exc ) { MessageBox(NULL, L"Blad podczas wysylania wiadomosci", L"Blad Klienta", MB_ICONERROR + MB_OK); }
}
//---------------------------------------------------------------------------
